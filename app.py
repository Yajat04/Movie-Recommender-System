...
def recommend(movie_input):
    ...
    recommended_movies = [
        movies.iloc[tup[0]]["title"]
        for tup in movie_list
    ]

    movie_ids = [
        movies.iloc[tup[0]]["id"]
        for tup in movie_list
    ]

    # Run the 5 API requests concurrently
    poster_urls = asyncio.run(
        fetch_posters(movie_ids)
    )
